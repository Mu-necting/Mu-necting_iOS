...
class MusicClusterAnnotationView: MKAnnotationView{
    ...
    private func count(genre genre: Genre) -> Int {
        guard let cluster = annotation as? MKClusterAnnotation else {
            return 0
        }

        return cluster.memberAnnotations.filter { member -> Bool in
            guard let music = member as? MusicAnnotation else {
    ...
    override func prepareForDisplay() {
        ...
        if let cluster = annotation as? MKClusterAnnotation{
            ...
            image = drawRatio(0, to: total, fractionColor: nil, wholeColor: genreColor)
